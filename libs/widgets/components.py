...
from PyQt5.QtWidgets import QHBoxLayout, QAbstractItemView, QHeaderView, QTableWidgetItem

from .base import MFrame, MLabel, MTableWidget
from model import SongModel
from utils import darker, get_ms, measure_time


class LPGroupHeader(MFrame):
    def __init__(self, app, title=None, parent=None):
        super().__init__(parent)
        self._app = app

        self._layout = QHBoxLayout(self)  # set self to HBoxLayout
        self.title_label = MLabel(title, self)  # set title label
    ...
    def setup_ui(self):
        ...
        self.setFixedHeight(22)  # Fix high

    def set_header(self, text):
        self.title_label.setText(text)


class LPGroupItem(MFrame):
    clicked = pyqtSignal()
    ...
    def __init__(self, app, name=None, parent=None):
        ...
        self.is_selected = False  # flag
        self.is_playing = False

        self._layout = QHBoxLayout(self)
        self._flag_label = MLabel(self)
        self._img_label = MLabel(self)
        self._name_label = MLabel(name, self)

        self.setObjectName('lp_group_item')
        self._flag_label.setObjectName('lp_group_item_flag')
    ...
    @property
    def name(self):
        return str(self._name_label.text())
...
class MusicTable(MTableWidget):
    ...
    def __init__(self, app, rows=0, columns=6, parent=None):
        ...
        self._alignment = Qt.AlignLeft | Qt.AlignCenter
        ...
        # column 1 is stretchable
        self.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)

        self.cellDoubleClicked.connect(self.on_cell_db_click)
    ...
    def add_item(self, song_model):
        ...
        album_item = QTableWidgetItem(song_model.album)
        artist_item = QTableWidgetItem(song_model.artists)
        m, s = get_ms(song_model.length)
        length_item = QTableWidgetItem(m + ':' + s)
    ...
    def search(self, text):
        if not text:
            for i in range(self.rowCount()):
                self.showRow(i)
            return
        # function lower() turn str A to a
        for i, song in enumerate(self.songs):
            if text.lower() not in song.title.lower()\
                    and text not in song.album.lower()\
                    and text not in song.artists.lower():
                self.hideRow(i)
            else:
                self.showRow(i)

    # TODO: Key press event hot_key
    def keyPressEvent(self, event):
        self.setFocus()  # gain focus from cell widget if necessary
        key_code = event.key()
